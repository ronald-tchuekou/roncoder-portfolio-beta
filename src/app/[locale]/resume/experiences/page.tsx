	...
	return (
		<section className='w-full flex flex-col gap-5'>
			...
			<RevealFromBottom elt={'p'} delay={0.1}>
				Ayant eu la possibilité de travailler pour plusieurs entreprise qui font dans le domaines de la tech, je
				vous liste ci-dessous quelques unes:
			</RevealFromBottom>
				...
				{EXPERIENCES_LIST.map((experience, index) => (
					<RevealFromBottom key={experience.id} delay={index * 0.1}>
						<div className={cn('bg-card border border-input', 'rounded-lg p-5 size-full', 'flex flex-col gap-3')}>
							<h3 className={cn('text-xl font-bold text-white')}>{experience.title}</h3>
